...
public class StorageLocalProvider : AppProvider<Storage, int>, IStorageProvider
{
    ...
    IHostEnvironment hostEnvironment) : base(dbContext)
  {
    _logger = logger;
    _mapper = mapper;
    _pathLocalRoot = Path.Combine(hostEnvironment.ContentRootPath, SpotLightsConstant.StorageLocalRoot);
  }
  ...
  private async Task<string> WriteAsync(string path, Stream stream)
  {
    ...
    var virtualPath = $"{SpotLightsConstant.StorageLocalPhysicalRoot}/{path}";
